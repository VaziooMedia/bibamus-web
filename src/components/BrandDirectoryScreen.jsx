// ============================================================
// Écran "Marques" — même structure que BreweryDirectoryScreen
// (navigation pays → liste, recherche), sans pagination serveur
// ni "à proximité" (mêmes raisons : pas encore d'équivalent
// serveur côté marques). Adapté aux champs réels des marques :
// logo (pas d'avatar/emoji) et pays d'origine (originCountry).
// ============================================================
...
import { loadBrandsDirectory, COUNTRY_CODE_TO_LABEL } from "../data/sharedDirectories.js";

const countryLabel = (code) => COUNTRY_CODE_TO_LABEL[code] || code;

export function BrandDirectoryScreen({ myBrands = [], myBibroCode, isAdmin, addIntent, onBack, onOpenBrand, goToSubmit, activeCountry, setActiveCountry }) {
  ...
  const countryCounts = useMemo(() => {
    const counts = {};
    all.forEach((b) => {
      counts[b.originCountry] = (counts[b.originCountry] || 0) + 1;
    });
    return counts;
  }, [all]);
  ...
  const items = useMemo(() => {
    ...
    if (activeCountry) {
      return all.filter((b) => b.originCountry === activeCountry).sort((a, b) => a.name.localeCompare(b.name));
    }
    return [];
  }, [all, searching, q, activeCountry]);

  const alreadyAdded = (b) => myBrands.some((mb) => mb.isFavorite && mb.sourcePublicBrandId === b.id);
  ...
  const renderBrandRow = (b) => (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
        <EntityAvatar photoUrl={b.logoUrl} size={44} />
        <div>
          <div style={{ fontWeight: 700, fontSize: "15px", display: "flex", alignItems: "center", gap: "6px" }}>
            {b.name}
          </div>
          <div style={{ fontSize: "9.5px", color: COLORS.inkSoft, marginTop: "2px" }}>{countryLabel(b.originCountry)}</div>
        </div>
      </div>
  ...
  const title = searching ? "Marques" : activeCountry ? countryLabel(activeCountry) : "Marques";

  return (
    <div style={{ padding: "28px 20px", display: "flex", flexDirection: "column", flex: 1 }}>
      <PageHeader onBack={handleBack} />
      {addIntent && !activeCountry && !searching && (
        <span style={{ fontFamily: "'Urbanist', sans-serif", fontSize: "12px", letterSpacing: "2px", color: COLORS.wine, fontWeight: 700 }}>AJOUTER UNE MARQUE</span>
      )}
      ...
      {!activeCountry && !searching && addIntent && (
        <p style={{ fontSize: "13px", color: COLORS.inkSoft, marginBottom: "16px" }}>
          Vérifie d'abord si elle existe déjà, pour éviter les doublons — sinon tu pourras la créer en bas de cette page.
        </p>
      )}

      {addIntent && (
        <PrimaryButton onClick={goToSubmit} style={{ width: "100%", marginBottom: "16px" }}>
          Je ne la trouve pas, créer une fiche
        </PrimaryButton>
      )}
      ...
      {!addIntent && !activeCountry && !searching && (
        <div style={{ display: "flex", justifyContent: "center", marginBottom: "16px" }}>
          <button
            ...
            aria-label="Proposer une marque"
          >
            +
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "20px" }}>
          {items.length === 0 && <p style={{ color: COLORS.inkSoft, fontSize: "14px", fontStyle: "italic" }}>Aucune marque trouvée.</p>}
          {items.map(renderBrandRow)}
        </div>
      ) : activeCountry ? (
        <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "20px" }}>
          {items.length === 0 && <p style={{ color: COLORS.inkSoft, fontSize: "14px", fontStyle: "italic" }}>Aucune marque pour l'instant.</p>}
          {items.map(renderBrandRow)}
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "20px" }}>
          {countries.length === 0 && <p style={{ color: COLORS.inkSoft, fontSize: "14px", fontStyle: "italic" }}>Aucune marque enregistrée pour l'instant.</p>}
